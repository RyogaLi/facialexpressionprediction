...
def TrainNN(num_hiddens, eps, momentum, num_epochs):
  ...
  for epoch in xrange(num_epochs):
    ...
    # PART 2.2 : compute cross entropy using mean classification error
    error = np.abs(target_train - prediction)
    train_CE = np.where(error > 0.5)[1].size / float(target_train.shape[1])
    ...
    # Error Rate
    valid_CE = EvaluateError(inputs_valid, target_valid, W1, W2, b1, b2)
...
def Evaluate(inputs, target, W1, W2, b1, b2):
  ...
  # CE = np.where(error > 0.5)[1].size / float(target.shape[1])
  
  return CE

def EvaluateError(inputs, target, W1, W2, b1, b2):
  """Evaluates the model on inputs and target."""
  h_input = np.dot(W1.T, inputs) + b1  # Input to hidden layer.
  h_output = 1 / (1 + np.exp(-h_input))  # Output of hidden layer.
  logit = np.dot(W2.T, h_output) + b2  # Input to output layer.
  ...
  # PART 2.2 : compute cross entropy using mean classification error
  error = np.abs(target - prediction)
  ER = np.where(error > 0.5)[1].size / float(target.shape[1])
  
  return ER


def DisplayCEPlot(train_error, valid_error):
  plt.figure(1)
  plt.clf()
...
def main():
  num_hiddens = 100 # PART 2.4 2;5 and 30;100
  eps = 0.02 # PART 2.3: 0.01;0.2;0.5
  # eps = 0.02
  # momentum = 0.5
  momentum = 0.5 # PART 2.3: 0.0;0.5;0.9
  num_epochs = 1000
  ...
  DisplayCEPlot(train_error, valid_error)
